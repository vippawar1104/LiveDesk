...
class TwilioHandler:
    ...
    async def handle_media_stream(self, websocket):
        ...
        video_input_queue = (
            asyncio.Queue()
        )  # Not used for Twilio but required by GeminiLive
        ...
        async def audio_output_callback(data):
            ...
            try:
                ...
                resampled_data, _ = audioop.ratecv(
                    intermediate, 2, 1, 16000, 8000, None
                )
        ...
        try:
            async for message in websocket.iter_text():
                ...
                if event == "start":
                    ...
                    logger.info(
                        f"Twilio Stream started — streamSid={self.stream_sid}, callSid={call_sid}"
                    )
                    logger.info(
                        f"Stream metadata: {json.dumps(data['start'], indent=2)}"
                    )
            ...
            if gemini_task.done() and not gemini_task.cancelled():
                ...
                if exc:
                    logger.error(
                        f"Gemini task failed with exception: {exc}", exc_info=exc
                    )
            gemini_task.cancel()
